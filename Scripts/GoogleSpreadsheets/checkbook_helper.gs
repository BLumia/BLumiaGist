...
function fillActivedRowColorAndBalance() {
  var sheet = SpreadsheetApp.getActiveSheet();
  var range = sheet.getDataRange();
  var activeRowIndex = sheet.getActiveCell().getRowIndex();
  setCellBackground(range.getCell(activeRowIndex, 3));
  range.getCell(activeRowIndex, 4).setFormula("=INDIRECT(\"R[-1]C[0]\",false)+INDIRECT(\"R[0]C[-1]\",false)");
}