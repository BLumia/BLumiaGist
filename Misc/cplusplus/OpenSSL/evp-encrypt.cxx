...
void hex_to_bin(const secure_string &hexStr, const int length, byte * buf);
...
int main(int argc, char* argv[])
{
    ...
    puts("--------------");
    printf("%10s:       123456789 123456789 123456789 123456789\n", "ruler");
    secure_string ivStr = bin_to_hex(iv, BLOCK_SIZE);
    printf("%10s:       %s\n", "iv", ivStr.c_str());
    secure_string keyStr = bin_to_hex(key, KEY_SIZE);
    printf("%10s:       %s\n", "key", keyStr.c_str());
    puts("--------------");
    
    byte keyFromStr[KEY_SIZE], ivFromStr[BLOCK_SIZE];
    hex_to_bin(ivStr, BLOCK_SIZE, (byte*)ivFromStr);
    hex_to_bin(keyStr, KEY_SIZE, (byte*)keyFromStr);
    aes_decrypt(keyFromStr, ivFromStr, ctext, rtext);
