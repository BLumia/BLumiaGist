...
//Prepare
	...
	$sql=$pdo->prepare("select * from tb_topic where good_level<>0 order by Lastreply_Time desc limit $front,$PAGE_ITEMS");
	$sql->execute();
	$topicList=$sql->fetchAll();
	$topicCount=count($topicList);
	
	$sql=$pdo->prepare("select * from tb_topic where good_level<>0");
	$sql->execute();
	$totalTopic=$sql->fetchAll();
	$totalCount=count($totalTopic);
//Main Code
	require("./template/".$PAGE_TEMPLATE."/recommend.php");
?>
</html>
