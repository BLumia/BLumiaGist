...
GLfloat gFovVal = 45.0f, gViewX = 0.0f;
...
int main() 
{
	...
	while(!glfwWindowShouldClose(window)) {
		...
		view = glm::translate(view, glm::vec3(gViewX, 0.0f, -3.0f)); // view martix here
		projection = glm::perspective(glm::radians(gFovVal), (GLfloat)screenWidth / (GLfloat)screenHeight, 0.1f, 100.0f); //aspect-ratio here, tryed 4:3 and 16:9
		//+1s
		...
		for(GLuint i = 0; i < 10; i++)
		{
		  ...
		  model = glm::rotate(model, glm::radians(  i%3==0?((GLfloat)glfwGetTime() * 50.0f):0.0f + angle), glm::vec3(1.0f, 0.3f, 0.5f));
		  glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
